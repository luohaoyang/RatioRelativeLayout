...
class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private void setSizeParameters(double progressCircleWidth, double progressCircleHeight, double centerRadius,
    ...
    public void updateSizes(@ProgressDrawableSize int size) {
        if (size == LARGE) {
            setSizeParameters(CIRCLE_DIAMETER_LARGE, CIRCLE_DIAMETER_LARGE, CENTER_RADIUS_LARGE, STROKE_WIDTH_LARGE,
                ARROW_WIDTH_LARGE, ARROW_HEIGHT_LARGE);
        } else {
    ...
    void updateRingColor(float interpolatedTime, MaterialProgressDrawable.Ring ring) {
        if (interpolatedTime > COLOR_START_DELAY_OFFSET) {
            ...
            ring.setColor(evaluateColorChange((interpolatedTime - COLOR_START_DELAY_OFFSET)
                / (1.0f - COLOR_START_DELAY_OFFSET), ring.getStartingColor(), ring.getNextColor()));
        }
    }
    ...
    void applyFinishTranslation(float interpolatedTime, MaterialProgressDrawable.Ring ring) {
        ...
        final float startTrim =
            ring.getStartingStartTrim() + (ring.getStartingEndTrim() - minProgressArc - ring.getStartingStartTrim())
                * interpolatedTime;
        ring.setStartTrim(startTrim);
        ring.setEndTrim(ring.getStartingEndTrim());
        final float rotation =
            ring.getStartingRotation() + ((targetRotation - ring.getStartingRotation()) * interpolatedTime);
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
                if (mFinishing) {
                    ...
                    if (interpolatedTime > END_TRIM_START_DELAY_OFFSET) {
                        ...
                        final float endTrim =
                            startingEndTrim + (minArc * MATERIAL_INTERPOLATOR.getInterpolation(scaledTime));
    ...
    private static class Ring {
        ...
        public void draw(Canvas c, Rect bounds) {
            ...
            arcBounds.inset(mPaint.getStrokeWidth(), mPaint.getStrokeWidth());
